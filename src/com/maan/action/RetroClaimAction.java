...
import com.maan.service.RetroClaimService;
import com.opensymphony.xwork2.ActionContext;
import com.opensymphony.xwork2.ActionSupport;
import com.opensymphony.xwork2.ModelDriven;

public class RetroClaimAction extends ActionSupport implements ModelDriven<ClaimBean>{
	...
	private transient final RetroClaimService SERVICE=new RetroClaimService();
		...
		if(StringUtils.isNotBlank(bean.getProposal_No())){
		 editMode = dropDownController.EditModeStatus(bean.getProposal_No(),bean.getLayerNo());
			...
			bean.setOpendDate(bean1.getOpendDate());
				...
				if(bean.getPolicy_Contract_No()!=null &&"5".equalsIgnoreCase((String)session.get("mfrid")))
				{
...
/***	    private void claimPaymentValidation() {
		...
		if("4".equalsIgnoreCase(bean.getProductId()) && StringUtils.isNotBlank(bean.getDepartmentId()) && StringUtils.isNotBlank(bean.getContarctno())){
			int count =SERVICE.claimMasterValidation(bean,"class");
		if(count==0){
			addActionError("No Class was found for this Contract number");
		}
	    }
		if("5".equalsIgnoreCase(bean.getProductId()) && StringUtils.isNotBlank(bean.getLayerNo()) && StringUtils.isNotBlank(bean.getContarctno())){
					...
					if(!"N".equalsIgnoreCase(editMode) && StringUtils.isBlank(bean.getMultiuserError())){
						...
						/**if(StringUtils.isNotBlank(bean.getClaimDisplay()) && "claimDisplaypayment".equalsIgnoreCase(bean.getClaimDisplay())){
							if("contractidentifier1".equalsIgnoreCase(bean.getFlag())){
							 ...
							 if(StringUtils.isNotBlank(bean.getClaimDisplay()) && "claimDisplay".equalsIgnoreCase(bean.getClaimDisplay())){
							 if(bean.getBusinessMode().equalsIgnoreCase("Bmode") && null !=bean.getPaymentFlag() &&  ("new".equalsIgnoreCase(bean.getPaymentFlag()) ||"edit".equalsIgnoreCase(bean.getPaymentFlag())) ){
								 if(session.get("Cliam_cliam_no")!=null)
								   ...
								   if("new".equalsIgnoreCase(bean.getPaymentFlag()) || "edit".equalsIgnoreCase(bean.getPaymentFlag())){
			...
			/*if(StringUtils.isBlank(bean.getSubProfitId())){
				...
				if(bean.getBusinessMode().equalsIgnoreCase("Amode"))
				 ...
				 {	
					 ...
					 bean.setDeptId((String) session.get("DepartmentId")==null?"0":(String) session.get("DepartmentId"));
					 if("N".equalsIgnoreCase(editMode)){
					    reserveValidation();
					 }
						if(!hasActionErrors() && "N".equalsIgnoreCase(editMode))
		...
		public void claimsInsertValidation() {
			final Validation val = new Validation();
				...
				if(!"4".equalsIgnoreCase(productId)){
					...
					else if("5".equalsIgnoreCase(productId) && bean.getBasis()!=null && "2".equals(bean.getBasis())) {
				...
				}if(!"5".equalsIgnoreCase(bean.getProductId()) ){
					if(StringUtils.isBlank(bean.getSubProfitId())){
				...
				if(StringUtils.isNotBlank(bean.getGrossLossFGU()) && StringUtils.isNotBlank(bean.getLoss_Estimate_Orig_Curr())){
				...
				{
					bean.setPreReopenDate(new DropDownControllor().getpreReopendDate(bean.getContarctno(),bean.getClaim_No(),"RetroReopen"));
				...
				{
					String preDate = new DropDownControllor().getpreReopendDate(bean.getContarctno(),bean.getClaim_No(),"RetroReputed");
				...
				//else{
					...
					} else
					{
						...
						/*else {
							...
							String uwLimit=dropDownController.getUnderWriterLimmit(bean.getUnderwriter(),(String)session.get("processId"), productId, (String)session.get("DepartmentId"));
...
//}
		...
		public String submitReIns(){
			...
			value="<script type='text/javascript'>window.opener.retroclaims.reinstPremiumOCOS.value='"+bean.getReinsCalVal()+"'; window.opener.retroclaims.paid_Amount_Orig_curr.value='"+ val +"';window.opener.retroclaims.disabledFields.value='Y'; window.close();</script>" ; 
