...
import com.maan.dao.RetroClaimDAO;

public class RetroClaimDAOImpl extends MyJdbcTemplate implements RetroClaimDAO{
		...
		try {
			if(StringUtils.isNotBlank(beanObj.getClaim_No()))
			{
				...
				if("5".equalsIgnoreCase(beanObj.getProductId())){
					query=getQuery(DBConstants.CLAIM_SELECT_SUMPAIDAMT_RETRO);
					...
					query=getQuery(DBConstants.CLAIM_SELECT_REVSUMPAIDAMT_RETRO);
					LOGGER.info("Select Query==>"+query);
			...
			if(mode==1)
			{
				if("5".equals(beanObj.getProductId())){
					query=getQuery(DBConstants.CLAIM_SELECT_XOLORTREATYGETCLIMEQUERY);
				}
				else{
					query=getQuery("CLAIM_SELECT_RETRO_GETCLIMEQUERY");
				}
				LOGGER.info("Select Query==>"+query);
				args=new Object[5];
				...
				for(int i=0;i<list.size();i++){
					...
					if(StringUtils.isBlank(beanObj.getDepartmentId())){
					...
					//beanObj.setShareSigned(contractDetails.get("SHARE_SIGNED"))
						...
						beanObj.setSigned_Share(contractDetails.get("RSK_SHARE_SIGNED")==null?"0":contractDetails.get("RSK_SHARE_SIGNED").toString());
						...
						beanObj.setSumInsOSOC(getShareVal(contractDetails.get("RSK_LIMIT_OC")==null?"0":contractDetails.get("RSK_LIMIT_OC").toString(),beanObj.getSigned_Share(),"share"));
						beanObj.setSumInsOSDC(getShareVal(contractDetails.get("RSK_LIMIT_DC")==null?"0":contractDetails.get("RSK_LIMIT_DC").toString(),beanObj.getSigned_Share(),"share"));
					...
					String count="";
					if("4".equals(beanObj.getProductId())){
  		...
  		{	
  			query=getQuery(DBConstants.CLAIM_SELECT_CLAIMEDIT_RETRO);
	     	LOGGER.info("Select Query====>"+query);
	     	LOGGER.info("Arg[0]====>"+beanObj.getClaim_No());
	     	LOGGER.info("Arg[1]====>"+beanObj.getPolicy_Contract_No());
  		...
  		{
  			query=getQuery(DBConstants.CLAIM_SELECT_GETCLAIMRESERVELIST_RETRO);
  			LOGGER.info("Select Query=>"+query);
  			LOGGER.info("Arg[0]=>"+beanObj.getClaim_No());
  			LOGGER.info("Arg[1]=>"+beanObj.getPolicy_Contract_No());
  			this.mytemplate.query(query, new Object[] {beanObj.getClaim_No(),beanObj.getPolicy_Contract_No()},new RowMapper() {
				public Object mapRow(ResultSet reserveList, int rowNum) throws SQLException {
  				...
  				query=getQuery(DBConstants.CLAIM_SELECT_LOSSESTIMATEREVISEDOC_RETRO);
				LOGGER.info("Select Query====>"+query);
				LOGGER.info("Arg[0]====>"+beanObj.getClaim_No());
				LOGGER.info("Arg[1]====>"+beanObj.getPolicy_Contract_No());
  		...
  		{
  			query=getQuery(DBConstants.CLAIM_SELECT_GETCLAIMUPDATELIST_RETRO);
  			LOGGER.info("Select Query====>"+query);
  			LOGGER.info("Arg[0]====>"+beanObj.getClaim_No());
  			LOGGER.info("Arg[1]====>"+beanObj.getPolicy_Contract_No());
  			this.mytemplate.query(query, new Object[] {beanObj.getClaim_No(),beanObj.getPolicy_Contract_No()},new RowMapper() {
  		...
  		{
  			query=getQuery(DBConstants.CLAIM_SELECT_GETCLAIMREVIEWQUERY_RETRO);
  			LOGGER.info("Select Query=>"+query);
  			LOGGER.info("Arg[0]"+beanObj.getClaim_No());
  			LOGGER.info("Arg[1]"+beanObj.getPolicy_Contract_No());
	...
	public List cliamTableList(final ClaimBean beanObj,final int mode)	 {
		...
		try {
			...
			query=getQuery(DBConstants.CLAIM_SELECT_CLAIMTABLELIST_RETRO);
			LOGGER.info("Select Query====>"+query);
		...
		if(mode==2||mode==7)
		{
			...
			}else if(mode==7)
			{
				...
				query=getQuery(DBConstants.CLIME_SELECT_GETCLAIMRESERVELIST_RETRO);
			}
			LOGGER.info("Select Query=>"+query);
			int i=0;
		...
		if(mode==3)
		{
			query=getQuery(DBConstants.CLAIM_SELECT_GETCLAIMLIST_RETRO);
			LOGGER.info("Select Query====>"+query);
			LOGGER.info("Arg[0]====>"+beanObj.getClaim_No());
			LOGGER.info("Arg[1]====>"+beanObj.getPolicy_Contract_No());
			LOGGER.info("Arg[2]====>"+beanObj.getBranchCode());
		...
		if(mode==4)
		{
			...
			String que="SELECT MAX(SL_NO) FROM TTRN_RETRO_CLAIM_PAYMENT WHERE  CLAIM_NO=? AND  CONTRACT_NO=?";
		...
		if(claimlist.size()>0){
				for(int i=0;i<claimlist.size();i++){
					...
					bean.setStatus_of_claim((String)this.mytemplate.queryForObject(getQuery("RETRO_GET_STATUS_OF_CLAIM"),new Object[]{beanObj.getContarctno(),beanObj.getClaim_No(),beanObj.getLayerNo()},String.class));
		...
		if(mode==5)
		{
			query=getQuery(DBConstants.CLAIM_SELECT_GETCLAIMPAYMENTLIST_RETRO);
			LOGGER.info("Select Query====>"+query);
			LOGGER.info("Arg[0]====>"+beanObj.getClaim_No());
			LOGGER.info("Arg[1]====>"+beanObj.getPolicy_Contract_No());
	...
	public boolean insertCliamDetails(final ClaimBean beanObj,final int mode) {
		...
		try {
			if (mode == 2) {
				...
				query=getQuery(DBConstants.CLAIM_INSERT_CLIAMDETAILS_RETRO);
				LOGGER.info("Select Query====>"+query);
				int i=0;
				for(Object o:args){
					LOGGER.info("Arg["+i+"]====>"+o);
				...
				args[0]=GetSl_no(beanObj.getClaim_No(),beanObj.getPolicy_Contract_No(),"TTRN_RETRO_CLAIM_UPDATION");
				query=getQuery(DBConstants.CLAIM_INSERT_GETUPDATIONQUERY_RETRO);
				...
				{
					args = CliamDetailsAruguments(beanObj, mode);
					query=getQuery(DBConstants.CLAIM_UPDATE_CLIAMDETAILSUPDATE_RETRO);
			...
			{   
				...
				if(checking)
				{
					...
					args[0]=GetSl_no(beanObj.getClaim_No(),beanObj.getPolicy_Contract_No(),"TTRN_RETRO_CLAIM_PAYMENT");
					query=getQuery(DBConstants.CLAIM_SELECT_MAXRESVID_RETRO);
					...
					query=getQuery(DBConstants.CLAIM_INSERT_PAYMENT_RETRO);
					LOGGER.info("Insert Query====>"+query);
					int i=0;
					for(Object o:args){
						LOGGER.info("Arg["+i+"]====>"+o);
					...
					query=getQuery("CLAIM_UPDATE_PAYMENT_RETRO");
					 arg=new Object[22];
					...
					query=getQuery(DBConstants.CLAIM_UPDATE_TOTALAMTPAIDTILLDATE_RETRO);
					LOGGER.info("Update Query====>"+query);
					...
					beanObj.setRi_Recovery((String) this.mytemplate.queryForObject("select RI_RECOVERY from TTRN_RETRO_CLAIM_DETAILS where CLAIM_NO=? and CONTRACT_NO=?",new Object[]{beanObj.getClaim_No(),beanObj.getContarctno()},String.class));
			...
			if(mode==8)
			{
				...
				}else
				{
					...
					query=getQuery(DBConstants.CLAIM_UPDATE_CLOSECLAIM_RETRO);
					LOGGER.info("Update Query=>"+query);
			...
			{
				args=CliamDetailsAruguments(beanObj,mode);
				query=getQuery(DBConstants.CLAIM_UPDATE_TTRNCLAIMDETAILSRDANDRDB_RETRO);
				LOGGER.info("Update Query====>"+query);
				int i=0;
				for(Object o:args){
					LOGGER.info("Arg["+i+"]====>"+o);
	...
	private boolean Mode_Validation(final String paymentRequestNo,final String claimNo)
	{
		...
		boolean modes=false;
		String query=getQuery(DBConstants.CLAIM_SELECT_PAYMENTREQNO_RETRO);
		LOGGER.info("Select Query====>"+query);
	...
	public double businessValidaion(final ClaimBean formObj,final int mode) {
		...
		if(mode==1)
		{
			query=getQuery(DBConstants.CLAIM_SELECT_LOSSESTIMATEOSOC_RETRO);
			LOGGER.info("Select Query====>"+query);
		...
		if(mode==2)
		{
			query=getQuery(DBConstants.CLAIM_SELECT_SUMPAIDAMTOC_RETRO);
			LOGGER.info("Select Query====>"+query);
			LOGGER.info("Arg[0]====>"+formObj.getClaim_No());
			LOGGER.info("Arg[1]====>"+formObj.getPolicy_Contract_No());
			amount=Double.parseDouble((String)this.mytemplate.queryForObject(query,new Object[]{formObj.getClaim_No(),formObj.getPolicy_Contract_No()},String.class));
		...
		if(mode==3)
		{
			query=getQuery(DBConstants.CLIAM_SELECT_TOTALAMTPAIDTILLDATE_RETRO);
			LOGGER.info("Select query====>"+query);
			LOGGER.info("Arg[0]====>"+formObj.getClaim_No());
			LOGGER.info("Arg[1]====>"+formObj.getPolicy_Contract_No());
			amount=Double.parseDouble((String)this.mytemplate.queryForObject(query,new Object[]{formObj.getClaim_No(),formObj.getPolicy_Contract_No()},String.class));
		...
		if(mode==4){
			query=getQuery(DBConstants.CLAIM_SELECT_LOSSESTREV_RETRO);
			LOGGER.info("Select Query====>"+query);
		...
		if(mode==5){
			...
			try {
				...
				if("edit".equalsIgnoreCase(formObj.getPaymentFlag())){
					query=getQuery("CLAIM_LOSS_ESTIMATE_PAID_DIFFERENCE_EDIT_RETRO");
					...
					query=getQuery(DBConstants.CLAIM_LOSS_ESTIMATE_PAID_DIFFERENCE_RETRO);
					//query=getQuery("CLAIM_LOSS_ESTIMATE_PAID_DIFFERENCE_EDIT_RETRO");
			...
			try {
				if("edit".equalsIgnoreCase(formObj.getPaymentFlag())){
					query=getQuery("CLAIM_SAF_OS_SUM_DIFFERENCE_EDIT_RETRO");
					amount=Double.parseDouble((String)this.mytemplate.queryForObject(query,new Object[]{formObj.getPolicy_Contract_No(),formObj.getClaim_No(),formObj.getClaimPaymentNo(),formObj.getPolicy_Contract_No(),formObj.getClaim_No(),formObj.getPolicy_Contract_No(),formObj.getClaim_No()},String.class));

				}else{
					query=getQuery(DBConstants.CLAIM_SAF_OS_SUM_DIFFERENCE_RETRO);
					LOGGER.info("Select Query==>"+query);
					LOGGER.info("arg[0]=>"+formObj.getPolicy_Contract_No()+" args[1]=>"+formObj.getClaim_No());
					amount=Double.parseDouble((String)this.mytemplate.queryForObject(query,new Object[]{formObj.getPolicy_Contract_No(),formObj.getClaim_No(),formObj.getPolicy_Contract_No(),formObj.getClaim_No(),formObj.getPolicy_Contract_No(),formObj.getClaim_No()},String.class));
					}
			...
			try {
				if("edit".equalsIgnoreCase(formObj.getPaymentFlag())){
					...
					query=getQuery(DBConstants.CLAIM_OTHER_FEE_OS_SUM_DIFFERENCE_RETRO);
					LOGGER.info("Select Query==>"+query);
					LOGGER.info("arg[0]=>"+formObj.getPolicy_Contract_No()+" args[1]=>"+formObj.getClaim_No());
					amount=Double.parseDouble((String)this.mytemplate.queryForObject(query,new Object[]{formObj.getPolicy_Contract_No(),formObj.getClaim_No(),formObj.getPolicy_Contract_No(),formObj.getClaim_No(),formObj.getPolicy_Contract_No(),formObj.getClaim_No()},String.class));
					}
	...
	private String getExcRateForCliam(final String claimNo,final String policyContractNo) {
		...
		String excRate="";
		String query=getQuery(DBConstants.CLAIM_SELECT_EXCRATE_RETRO);
		LOGGER.info("Select query====>"+query);
		LOGGER.info("Arg[0]====>"+claimNo);
		LOGGER.info("Arg[1]====>"+policyContractNo);
	...
	public String getClaimStatus(ClaimBean formObj) {
		...
		String status="";
		String query=getQuery(DBConstants.CLAIM_SELECT_CLAIMSTATUS_RETRO);
		LOGGER.info("Select query=>"+query);
	...
	public String getClaimDates(ClaimBean formObj,int mode)
	{
		...
		try{
			if(mode==1)
			{
				String query=getQuery(DBConstants.CLAIM_SELECT_GETINSDATE_RETRO);
				LOGGER.info("Select Query =>"+query);
			...
			{
				String query=getQuery(DBConstants.CLAIM_SELECT_GETLOSSDATE_RETRO);
				LOGGER.info("Select Query =>"+query);
			...
			{
				String query=getQuery(DBConstants.CLAIM_SELECT_GETLASTPAYMENTDT_RETRO);
				LOGGER.info("Select Query =>"+query);
	...
	public String getCurrencyID(String claimNo, String contractNo,String branchCode) {
		String Currencyid="";
		Currencyid=(String)this.mytemplate.queryForObject(getQuery("GET_CURRENCY_RETRO"), new Object[] {claimNo,contractNo,branchCode}, String.class);
		return Currencyid;
	}
	...
	public String cedentNumberCheck(ClaimBean bean) {
		...
		try{
			...
			query=getQuery("CEDENT_BASED_MAX_CLAIM_NUMBER_RETRO");
			args=new Object[2];
	...
	public int claimpaymentCount(ClaimBean bean) {
		int count=0;
		try{
		  String query=getQuery("GET_CLAIM_COUNT_RETRO");
	...
	public int insertAggregate(final ClaimBean beanObj) {
		...
		int spresult=0;
		try {
			query=getQuery("INSERT_ACCUMULATION_DATA_RETRO");
	...
	public List<Map<String, Object>> XlPremiumList(ClaimBean bean, String countryId) {
		...
		String currency="";
		try{
			query=getQuery("RETRO_XL_COST_LIST");
	...
	public  List<Map<String, Object>> ClaimPaidList(ClaimBean bean, String countryId) {
		...
		String currency="";
		try{
			query=getQuery("XL_CLAIM_DETAILS_RETRO");
			args = new Object[7];
			args[0] = bean.getProductId();
	...
	private double getDateDetails(ClaimBean bean) {
		...
		List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
		try{
			query=getQuery("GET_DATE_OF_LOSS_RETRO");
			args= new Object[4];
	...
	public String getProposalNo(ClaimBean bean) {
		...
		else if("5".equals(bean.getProductId())){
		...
		if(list!=null &&list.size()>0){
			...
			if(!"4".equalsIgnoreCase(bean.getProductId())){
	...
	public void claimPaymentEdit(ClaimBean bean) {
		try{
			List<Map<String,Object>> list =new ArrayList<Map<String,Object>>();
			String query=getQuery("GET_CLAIM_PAYMENT_DATA_RETRO");
	...
	public String getContractNO(ClaimBean bean) {
			...
			Object[] obj=null;
			try{
			query=getQuery("GET_CONTRACT_NUMBER_RETRO");
...
private String getShareVal(String lossAdvise, String signedShare, String type) {
	...
	/**public int claimMasterValidation(ClaimBean bean, String type) {
		...
		try{
			...
			args	= new Object[1];
			args[0]= bean.getClaim_No();
			query="SELECT COUNT(*) FROM TTRN_RETRO_CLAIM_DETAILS WHERE CLAIM_NO=?";
			count = this.mytemplate.queryForInt(query,args);
			}
